...
async function findData(dir: string, predicate: (data: any) => boolean): Promise<any[]> {
...
export const FileStorage = {
    ...
    async deleteNumerology(id: string): Promise<boolean> {
        try {
            ...
            await fs.writeFile(path.join(NUMEROLOGY_DIR, `${id}.json`), JSON.stringify(filteredReadings, null, 2));
            return true;
        } catch (error) {
